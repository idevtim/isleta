...
/// only exist as literals at `activityText` call sites — there is no registry to enumerate — so the
...
@Suite struct LocalizationCoverageTests {
    ...
    static let lookupFunction = "activityText"
    ...
    /// test ever reported was a demand to translate `timer.paused`, a key that does not exist.
    ///
    ...
    static let definitionFileName = "ActivityText.swift"

    static var moduleDirectory: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()   // IslandActivitiesTests
            .deletingLastPathComponent()   // Tests
            .deletingLastPathComponent()   // package root
            .appendingPathComponent("Sources/IslandActivities")
    }
