...
// ISLETA FORK. See queue.m — the audio format of the playing entry.
NSString *kMRAAudioFormat = @"audioFormat";
...
// ISLETA FORK. The playback queue.
//
// The per-item keys are deliberately *not* new spellings: an item's title is a title and an item's
// artist is an artist, so a queue entry reuses `kMRATitle`, `kMRAArtist`, `kMRAAlbum`,
// `kMRADuration` and `kMRAContentItemIdentifier` and a consumer needs one parser rather than two
// that agree until somebody renames one of them. Only the two concepts that genuinely have no
// now-playing equivalent get their own key.
//
// Note what is **absent** on purpose. `kMRAQueueIndex` and `kMRATotalQueueCount` are already in
// this file and are the two fields named after the queue; both come back missing from every
// payload, measured against a 36-item Music queue. A queue reader that trusts them reads nothing.
NSString *kMRAQueueItems = @"queueItems";
NSString *kMRAITunesStoreIdentifier = @"iTunesStoreIdentifier";
