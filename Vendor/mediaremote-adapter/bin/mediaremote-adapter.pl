...
FUNCTION:
  ...
  queue    Prints a window of the current playback queue once
...
OPTIONS:
  send
    --offset=N: The playback queue offset, for PlayItemInPlaybackQueue (131).
      Required together with --content-item-id; the offset on its own returns
      success and changes nothing.
    --content-item-id=ID: The queue entry's "contentItemIdentifier", as
      reported by "queue". The plural spelling of this key is inert.
    --track-id=ID, --station-id=ID, --station-hash=H: For LikeTrack (106) and
      BanTrack (107). Whichever of them the player reports.
  get
  ...
  queue
    --length=N: How many queue entries to ask for (5 by default). Entry 0 is
      always the track that is playing, so entry 1 is the next song. Asking for
      the whole queue is not offered: a music library on shuffle is tens of
      thousands of items.
  stream
    --queue: Emits {"type":"queue","queueItems":[...]} lines alongside the
      {"type":"data"} ones, refreshed whenever the player says the queue
      changed. Off by default, so output is unchanged for consumers that have
      never heard of it.
    --length=N: The queue window for --queue. Ignored without it. With --queue
      this is only the *opening* window: a line of "length N" on stdin re-asks
      with a new one, and a bare "queue" re-asks with the current one. stdin is
      read only with --queue, and end of file on it does not stop the stream.
  get, stream
...
if ($function_name eq "send") {
  # ISLETA FORK. Options on "send", so a command can carry a userInfo dictionary.
  # Parsed before the ID is shifted, because parse_options splices the flags out
  # of @ARGV wherever they appear and the ID is then whatever is left.
  my $options = parse_options(0);
  foreach my $key (keys %{$options}) {
    if ($key eq "offset"
      || $key eq "content-item-id"
      || $key eq "track-id"
      || $key eq "station-id"
      || $key eq "station-hash") {
